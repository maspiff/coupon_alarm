...
site_board_map = {
    'ppomppu': ['ppomppu', 'ppomppu4', 'ppomppu8', 'money', 'coupon'],
    'clien': ['allsell', 'jirum'],
    'ruriweb': ['1020', '600004'],
    'coolenjoy' : ['jirum']
}
...
class ModuleBasic(PluginModuleBase):
    def __init__(self, P):
        ...
        self.db_default = {
            ...
            'use_coupon_alarm': 'False',
            'use_coupon_keyword_alarm': 'False',
            'use_coupon_keyword_alarm_dist' : 'False',
            'coupon_alarm_keyword': '',
    ...
    def scrap_items(self):
        ...
        if P.ModelSetting.get('use_site_ppomppu') == 'True':
            boards = ['ppomppu', 'ppomppu4', 'ppomppu8', 'money', 'coupon']
    ...
    def process_discord_data(self):
        ...
        if msg_template is None or len(msg_template) == 0:
            return
        for item in items:
            if P.ModelSetting.get_bool('use_coupon_alarm'):
                ...
                keywords = P.ModelSetting.get('coupon_alarm_keyword').split(',')
                if P.ModelSetting.get_bool('use_coupon_alarm'):
                    is_send = True
                else:
                    is_send = False
                ...
                for keyword in keywords:
                    if P.ModelSetting.get_bool('use_coupon_keyword_alarm'):
                        if len(keyword) > 0 and keyword in title:
                            is_send = True
                    if P.ModelSetting.get_bool('use_coupon_keyword_alarm_dist'):
                        if len(keyword) > 0 and keyword in title:
                            is_dist_send = True
